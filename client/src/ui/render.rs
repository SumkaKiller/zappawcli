...
use crate::text::{pad_to_width, visible_width, wrap_text};
use crossterm::{
    cursor, execute,
...
/// Result of the authentication prompt.
pub struct AuthInfo {
...
/// Interactive auth prompt: server URL, username, password, register/login choice.
pub fn prompt_auth_info() -> io::Result<AuthInfo> {
    ...
    let (_term_w_u, term_h) = terminal::size().unwrap_or((80, 24));
    let start_y = term_h.saturating_sub(art.len() as u16 + 10) / 2;
    ...
    let subtitle = "A U T H E N T I C A T I O N";
    ...
    let prompt_x = term_w.saturating_sub(36) / 2;
    ...
    execute!(
        ...
        SetForegroundColor(Color::Red),
        Print("Username:                "),
        cursor::MoveTo(prompt_x + 10, start_y + art.len() as u16 + 5),
    )?;
    stdout.flush()?;
    ...
    execute!(
        ...
        Print("Password:                "),
        cursor::MoveTo(prompt_x + 10, start_y + art.len() as u16 + 6),
    )?;
    stdout.flush()?;
    ...
    execute!(
        stdout,
        cursor::MoveTo(prompt_x, start_y + art.len() as u16 + 8),
        SetForegroundColor(Color::Yellow),
        Print("[R]egister or [L]ogin? (R/L): "),
    )?;
    stdout.flush()?;
    let mut choice = String::new();
    io::stdin().read_line(&mut choice)?;
    let is_register = choice.trim().to_lowercase().starts_with('r');
...
pub fn render(f: &mut Frame, app: &App) {
    ...
    let status_text = if std::time::Instant::now() <= app.status_until {
        ...
        "Commands: /help /clear /save /refresh".to_string()
    };
    ...
    let header_content = ratatui::text::Line::from(vec![
        ...
        Span::styled(
            format!(" | {} @ {} | ", app.username, app.server_url),
    ...
    let input_paragraph = Paragraph::new(Line::from(line_spans)).block(input_block);
...
fn render_messages(f: &mut Frame, app: &App, area: Rect) {
    ...
    for msg in &app.messages {
        ...
        // Auto-wrap using the robust unicode function!
        let text_lines = wrap_text(&msg.text, max_bubble_w as usize);

        let text_width_max = text_lines
            .iter()
            .map(|l| visible_width(l))
            .max()
            .unwrap_or(0)
            .max(1);
        let bubble_width = text_width_max as u16 + 4;

        let color = if msg.is_mine {
            RColor::Red
        } else {
            RColor::LightRed
        };
        let align = if msg.is_mine {
            Alignment::Right
        } else {
            Alignment::Left
        };
        ...
        if !msg.is_mine {
            ...
            rendered_lines.push((label, RColor::DarkGray, Alignment::Left, 0));
        }

        // Top border
        let t_border = format!("╭{}╮", "─".repeat(text_width_max + 2));
        rendered_lines.push((t_border, color, align, slide_offset));

        for line in text_lines {
            let content = format!("│ {} │", pad_to_width(&line, text_width_max));
            rendered_lines.push((content, color, align, slide_offset));
        }

        // Bottom border
        let b_border = format!("╰{}╯", "─".repeat(text_width_max + 2));
        rendered_lines.push((b_border, color, align, slide_offset));

        // Spacing
        rendered_lines.push((String::new(), RColor::Reset, Alignment::Left, 0));
    }
    ...
    for (i, (line, color, align, slide_offset)) in rendered_lines.into_iter().enumerate() {
        ...
        let x_slide = match align {
            ...
            Alignment::Right => {
                let pad = inner_area
                    .width
                    .saturating_sub(visible_width(&line) as u16);
