...
pub fn run_command(app: &mut App, line: &str) -> String {
    ...
    match cmd.as_str() {
        ...
        "nick" => {
            if arg.is_empty() {
                String::from("usage: /nick NAME")
            } else {
                app.my_nick = arg.to_string();
                format!("nickname changed to {}", app.my_nick)
            }
        }
        ...
        "demo" => {
            app.messages.push(Message {
                text: String::from("demo: compact bubble mode"),
                is_mine: false,
                created: Instant::now(),
            });
            app.messages.push(Message {
                text: String::from("demo: smooth terminal UI"),
                is_mine: true,
                created: Instant::now(),
            });
