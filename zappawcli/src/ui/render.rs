...
pub fn bubble_lines(text: &str, max_bubble_w: usize) -> (Vec<String>, usize) {
    ...
    let mut content_w = 0;
    for line in &wrapped {
        content_w = content_w.max(visible_width(line));
    }
...
pub fn layout_message_lines(
    msg: &Message,
    term_w: u16,
    chat_w: u16,
) -> (Vec<String>, usize, u16, Color, Color) {
    ...
    let slide = if progress >= 1.0 {
        0u16
    } else {
        ((1.0 - progress) * if msg.is_mine { 6.0 } else { 4.0 }) as u16
    };
...
fn command_help_text() -> &'static str {
    "Commands: /help /clear /nick /save /me /demo /top /bottom"
}
...
pub fn render(stdout: &mut Stdout, app: &App) -> io::Result<()> {
    ...
    let status = if std::time::Instant::now() <= app.status_until {
        app.status.clone()
    } else {
        command_help_text().to_string()
    };
    
    let status_w = visible_width(&status) as u16;
    if status_w + 2 < term_w {
        draw_text(stdout, term_w - status_w - 2, 1, &status, Color::DarkGrey)?;
    }
    ...
    for (idx, (lines, bubble_w, slide, frame_color, body_color)) in items.iter().enumerate() {
        ...
        let base_x = if msg.is_mine {
            term_w.saturating_sub(*bubble_w as u16 + 2)
        } else {
            1
        };
    ...
    if !placeholder.is_empty() {
        draw_text(stdout, 2, usable_input_y, placeholder, Color::DarkGrey)?;
    }
    ...
    let hint = if app.help_visible() {
        ...
        "Enter - send  |  Ctrl+Q - quit  |  Ctrl+H - help" 
    };
    ...
    queue!(
        stdout,
        cursor::MoveTo(cursor_pos.min(term_w.saturating_sub(2)), usable_input_y),
        cursor::Show
    )?;
    ...
    if help_height > 0 {
        ...
        if bar_w > 2 {
            queue!(
                stdout,
                cursor::MoveTo(bar_x, bar_y.saturating_sub(1)),
